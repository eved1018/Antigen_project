...
def test():
    predictors = ['vorffip']
    df = pd.read_csv("/Users/moshe/Desktop/Research_Antigen/Vorffip/Unbound_work/Data/unbound_vorffip_results.csv")
...
def pml_maker(protein,df,cutoff_csv,folder,path,predictors,results_folder):
    ...
    folder_11 = Path("/Users/moshe/Desktop/Research_Antigen/Updated_PDBs_and_Data/unbound_pdbs copy")
    for testing in folder_11.iterdir():
        if protein_bound in str(testing.name):
            protein_name = str(testing.name[19:23])
            chain_name = str(testing.name)[24:25]
    ...
    for predictor in predictors:
        ...
        with open(f"/Users/moshe/Desktop/Research_Antigen/pymol/unbound_pred_list/{str(testing.name)}_predicted_list_pymol.txt", "a") as f:
            f.write(pred_res_list)
        with open(f"/Users/moshe/Desktop/Research_Antigen/pymol/unbound_annotated_list/{str(testing.name)}_annotated_list_pymol.txt", "a") as f:
