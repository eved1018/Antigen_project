...
def analyses(params) -> tuple:
    ...
    test_string = str(fscore_mcc_per_protein)
    test_string = test_string.replace(" ","").replace("(",",").replace(")","").replace("protein","").replace("dtype:object", "").strip()
    try:
        os.mkdir(f"/Users/moshe/Desktop/Research_MetaDPI/MetaDPIv2-main/metadpi/{output_path_dir}/fscore_by_method/")
    except:
        pass
    with open (f"/Users/moshe/Desktop/Research_MetaDPI/MetaDPIv2-main/metadpi/{output_path_dir}/fscore_by_method/{pred}.csv", "a") as outfile11:
        outfile11.write(f"{test_string}\n")
