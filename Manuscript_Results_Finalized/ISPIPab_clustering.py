...
output_input = "{INSERT THE NAME OF THE OUTPUT FOLDER FOR ISPIPab}"
...
df = pd.read_csv(f".../ISPIP-main/Data/output/{output_input}/bin_frame.csv")
...
cutoff_path = ".../ISPIP-main/Data/unbound_cutoff.csv"

#ISPIPab outputs folder with fscores for each method
path_starter = Path(f".../ISPIP-main/Data/output/{output_input}/fscore_by_method")
for file_starter in path_starter.iterdir():
    
    predictor = "TYPE PREDICTOR OF INTEREST"
    predictors =[predictor]
    fscore_file = file_starter
    #create directory to host clustering results
    os.mkdir(f"...../{predictor}")
    ...
    for item_predictor in predictors:
        for protein in proteins:
            ...
            os.mkdir(f".../{predictor}/{protein}")
            os.mkdir(f".../{predictor}/{protein}/predicted_residues/")

            with open(f".../{predictor}/{protein}/{protein}_unbound_predicted_residues.txt", 'a') as f:
    ...
    pdb_folder = Path("INSERT PATH TO PDBs OF ANTIGENS OF INTEREST TO CAPTURE RESIDUE DATA")

    pred_annotated_folder = Path(f"INSERT PATH TO CLUSTERING RESULTS FOR THE PREDICTOR OF INTEREST TO PULL THE RESIDUES OF INTEREST")
                                        ...
                                        with open(item_1) as annotated_file:
                                            for line1 in annotated_file:
                                                ...
                                                if (str(annotated_res_num) == str(res_num)): #change between pred and annotated
                                                    ...
                                                    with open(f".../{predictor}/{item.name}/predicted_residues/{res_num}.txt", 'a') as f:
    ...
    folder_infile_1 = Path(f".... PATH TO CLUSTERING RESULTS/{predictor}")
    ...
    os.mkdir(f"PATH TO CLUSTERING RESULTS/{predictor}/kmeans_cluster_1")
    os.mkdir(f"PATH TO CLUSTERING RESULTS/{predictor}/kmeans_cluster_2")
    os.mkdir(f"PATH TO CLUSTERING RESULTS/{predictor}/kmeans_cluster_3")
    os.mkdir(f"PATH TO CLUSTERING RESULTS/{predictor}/kmeans_cluster_4")
    os.mkdir(f"PATH TO CLUSTERING RESULTS/{predictor}/kmeans_cluster_5")
            ...
            with open (cutoff_path) as infile_4:
                for cluster in infile_4:
                    if cluster.strip().split(",")[0] == protein_name:
                        ...
                        with open (f".../{predictor}/kmeans_cluster_1/{protein_name}_{predictor}.txt", "a") as outfile_1:
                            outfile_1.write(f"{string_clu1}")
                        with open (f".../{predictor}/kmeans_cluster_2/{protein_name}_{predictor}.txt", "a") as outfile_2:
                            outfile_2.write(f"{string_clu2}")
                        with open (f".../{predictor}/kmeans_cluster_3/{protein_name}_{predictor}.txt", "a") as outfile_3:
                            outfile_3.write(f"{string_clu3}")
                        with open (f".../{predictor}/kmeans_cluster_4/{protein_name}_{predictor}.txt", "a") as outfile_4:
                            outfile_4.write(f"{string_clu4}")
                        with open (f".../{predictor}/kmeans_cluster_5/{protein_name}_{predictor}.txt", "a") as outfile_5:
                        ...
                        with open(f".../{predictor}/{predictor}_kmeans_clustering_results.csv", "a") as outfile:
