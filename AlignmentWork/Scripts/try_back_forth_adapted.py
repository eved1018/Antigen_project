...
def main(unbound_map, bound_map, bound_name):
                    ...
                    for line in alignment_text:
                        ...
                        if pos_num == bound_pos_num:
                            ...
                            with open(unbound_map) as unbound_map_file: #unbound .map file
                                ...
                                unbound_name11 = str(Path(unbound_map))
                                unbound_name = str(unbound_name11[-20:-16])
                                ...
                                for line in unbound_map_text:
                                    ...
                                    try:
                                        ...
                                        if unbound_pos_num == unbound_pos_num_map and unbound_one_letter_AA == one_letter_AA:
                                            ...
                                            with open(f"/Users/moshe/Desktop/Research_Antigen/Updated_PDBs_and_Data/unbound_annotated_results/{bound_name}_{unbound_name}_annotated.txt", 'a') as f:
...
for folder in paired_folder.iterdir():
    for file in folder.iterdir():
        if str(file).endswith("_unbound_map.csv"):
            unbound_map = file
            #print(unbound_map)
        if str(file).endswith("_bound_map.csv"):
            bound_map = file
            bound_name11 = str(Path(bound_map))
            bound_name = str(bound_name11[-18:-14])
        if "annotated_results" in str(file):
            annotated_results_file = file
        if "msa_aligned" in str(file):
            msa2column_file = file
    main(unbound_map, bound_map, bound_name)
