...
def bound_get_fasta_from_map(bound_map):
    with open(bound_map) as bound_map_file:
        ...
        return combined_bound, old_bound_name
...
paired_folder = Path("/Users/moshe/Desktop/Research_Antigen/Updated_PDBs_and_Data/Pairs_Updated copy")
for folder in paired_folder.iterdir():
    ...
    f = open(f"{folder}/{bound_get_fasta_from_map(bound_map)[1]}_{unbound_get_fasta_from_map(unbound_map)[1]}_fasta_compiled.txt", "w")
